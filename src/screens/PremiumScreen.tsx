...
export function PremiumScreen({ language, store, onBack, onUnlocked }: { language: Language; store: PremiumStoreController; onBack: () => void; onUnlocked: () => void }) {
  ...
  const subscriptionsUrl = Platform.OS === 'ios'
    ...
    : 'https://play.google.com/store/account/subscriptions?package=com.kamilunavo.navokids';
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <Text style={styles.legal}>{language === 'de'
        ? 'Monats- und Jahresabos verlängern sich automatisch, bis sie im Store gekündigt werden. Die Abrechnung erfolgt ausschließlich im geschützten Elternbereich über den App Store oder Google Play.'
        : 'Monthly and yearly subscriptions renew automatically until cancelled in the store. Billing is handled only in the protected parent area through the App Store or Google Play.'}</Text>
      <View style={styles.legalLinks}>
        <Pressable accessibilityRole="link" onPress={() => openUrl('https://kamilunavo.com/navokids/datenschutz')} style={styles.legalLinkButton}>
          <Text style={styles.legalLink}>{language === 'de' ? 'Datenschutz' : 'Privacy Policy'}</Text>
        </Pressable>
        <Pressable accessibilityRole="link" onPress={() => openUrl('https://kamilunavo.com/navokids/nutzungsbedingungen')} style={styles.legalLinkButton}>
          <Text style={styles.legalLink}>{language === 'de' ? 'Nutzungsbedingungen' : 'Terms of Use'}</Text>
        </Pressable>
        <Pressable accessibilityRole="link" onPress={() => openUrl(subscriptionsUrl)} style={styles.legalLinkButton}>
          <Text style={styles.legalLink}>{language === 'de' ? 'Abo verwalten' : 'Manage subscription'}</Text>
        </Pressable>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  restore: { padding: 14, marginTop: 7 }, restoreText: { color: colors.tealDark, fontSize: 14, fontWeight: '900', textDecorationLine: 'underline' }, legal: { color: colors.muted, maxWidth: 500, textAlign: 'center', fontSize: 10, lineHeight: 15, marginTop: 4 },
  legalLinks: { width: '100%', maxWidth: 500, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', marginTop: 8 },
  legalLinkButton: { minHeight: 36, justifyContent: 'center', paddingHorizontal: 9, paddingVertical: 6 },
  legalLink: { color: colors.tealDark, fontSize: 11, lineHeight: 15, fontWeight: '800', textAlign: 'center', textDecorationLine: 'underline' },
